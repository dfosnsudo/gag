document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadNewContent(username, selectedCards) {
        const loadingCircles = document.querySelectorAll('.loading-circle');
         ...
         const phases = [
            { texts: ['Connecting to servers.', 'Successfully Connected.'], color: '#1ec962', bouncingIndex: 1 },
            { texts: ['Finding Username.', 'Username Found.'], color: '#1ec962', bouncingIndex: 2 },
            { texts: ['Generating Items.', 'Starting Transfer.'], color: '#1ec962', bouncingIndex: 3 },
            { texts: ['Verifying if you are part of our group.', 'Verification successful, your rewards have been sent to your account.'], color: '#1ec962', bouncingIndex: 4, }
        ];

       function updateCircles(phase) {
    loadingCircles.forEach((circle, index) => {
        if (index < phase.bouncingIndex) {
            // Pallini precedenti: colore verde senza animazione di rimbalzo
            circle.style.backgroundColor = phase.color;
            circle.classList.remove('bounce');
        } else if (index === phase.bouncingIndex) {
            // Ultimo pallino: inizia a rimbalzare
            circle.style.backgroundColor = "#E8E8E8"; // Colore grigio iniziale
            circle.classList.add('bounce'); // Animazione di rimbalzo

            // Dopo il completamento dell'animazione di rimbalzo (2.5 secondi per esempio), diventa verde e smette di rimbalzare
            setTimeout(() => {
                circle.style.backgroundColor = "#1ec962"; // Diventa verde
                circle.classList.remove('bounce'); // Rimuove l'animazione di rimbalzo
            }, 2500); // Imposta un timeout per farlo diventare verde dopo il rimbalzo (puoi regolare il tempo in base alla durata dell'animazione)
        } else {
            // Pallini successivi: colore grigio senza animazione
    ...
    function createPopupCard(user) {
        ...
        const texts = [
            "Just Claimed a Racoon!",
            "Just Claimed Bone Blossom!",
            "Just Claimed a Butterfly Pet!",
            "Just Claimed a Kitsune!",
            "Just Claimed Bell Pepper!",
            "Just Claimed Cacao!",
            "Just Claimed Sugar Apple!",
            "Just Claimed Candy Blossom!",
            "Just Claimed Dragon Pepper!",
            "Just Claimed a Queen Bee!",
            "Just Claimed a Red Fox!",
            "Just Claimed a Corrupted Kitsune!",
            "Just Claimed a Disco Bee!",
            "Just Claimed a Dragonfly Pet!",
            "Just Claimed Ancient Pack!",
            "Just Claimed a Pterodactyl"
        ];
